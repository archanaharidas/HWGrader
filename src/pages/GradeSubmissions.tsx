...
const GradeSubmissions: React.FC = () => {
  ...
  const [feedback, setFeedback] = useState<{ [criterionId: string]: string }>({});
  ...
  const handleGradeClick = (submissionId: string) => {
    ...
    const existingGrade = gradedSubmissions.find(gs => gs.submissionId === submissionId);
    if (existingGrade) {
      const gradesObj: { [criterionId: string]: number } = {};
      const feedbackObj: { [criterionId: string]: string } = {};
  ...
  return (
    <div className="space-y-6">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {assignmentSubmissions.map(submission => (
                    <tr key={submission.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">{submission.studentId}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{submission.submissionDate}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        >
                          {isSubmissionGraded(submission.id) ? 'View/Edit Grade' : 'Grade'}
                        </button>
                      </td>
                    </tr>
                  ))}
      ...
      {isGradingPanelOpen && selectedSubmission && selectedAssignment && (
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-semibold text-gray-800">Grading Panel</h2>
            <button
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-medium text-gray-800 mb-2">Submission Details</h3>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedSubmission.content}</p>
                </div>
              </div>
            </div>

            <div>
              <h3 className="text-lg font-medium text-gray-800 mb-4">Grading Criteria</h3>
              <form onSubmit={handleSubmit} className="space-y-6">
                {selectedAssignment.criteria.map(criterion => (
                  <div key={criterion.id} className="border rounded-lg p-4">
                    <div className="flex justify-between items-center mb-2">
                      <h4 className="font-medium text-gray-800">{criterion.name}</h4>
                      <div className="text-sm text-gray-600">
                        {grades[criterion.id] || 0} / {criterion.maxPoints} points
                      </div>
                    </div>
                    <div className="mb-4">
                      <input
                        ...
                        onChange={e => handleGradeChange(criterion.id, parseInt(e.target.value))}
                        className="w-full"
                      />
